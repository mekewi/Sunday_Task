...
public class PipeMeshGenerator : MonoBehaviour {
    ...
    public void RenderPipe() {
        ...
        GameObject tubeParent = new GameObject("Tube");
        tubeParent.transform.SetParent(transform);
        var tubeBody = tubeParent.AddComponent<Rigidbody>();
        tubeBody.isKinematic = true;
        
        GameObject pipe = new GameObject("pipe");
        pipe.transform.SetParent(tubeParent.transform);
        var empty = Instantiate(tube_empty);
        empty.transform.SetParent(tubeParent.transform);
        // add mesh filter if not present
        MeshFilter mf = pipe.AddComponent<MeshFilter>();
        ...
        MeshRenderer mr = pipe.AddComponent<MeshRenderer>();
        mr.materials = new Material[1] { pipeMaterial };
        var pipeMeshCollider = pipe.AddComponent<MeshCollider>();
        pipeMeshCollider.sharedMesh = mesh;
    }
